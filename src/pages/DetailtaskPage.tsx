...
export default function DetailtaskPage() {
  ...
  >(null);
  const [currentStatus, setCurrentStatus] = useState<string>(task?.status || "К разработке");
  ...
  const currentUserName = useMemo(() => {
    ...
    if (currentUserId) {
      ...
      if (Array.isArray(users)) {
        const user = users.find((u: any) => String(u?.id) === String(currentUserId));
            ...
            <Stack flex="2" w="full" spacing={6}>
              ...
              >
                <HStack mb={4} spacing={2}>
                  <Icon as={FiMessageSquare} color={accentColor} boxSize={4} />
                  ...
                  >
                    <HStack spacing={3}>
                      {isTeamLoading ? (
                        ...
                        <>
                          <Icon as={FiUser} color={subtextColor} boxSize={4} />
                          <Text
                            fontSize="sm"
                            color={subtextColor}
                            fontStyle="italic"
